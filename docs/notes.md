...
- Genuine gap: you can't confiscate / take equipped gear from players. — **OPEN.** `confiscate()` only ever sees an `InventoryItem`; equipped gear lives in `character.equipped` slots, which that flow never reaches. Needs a DM surface listing equipped items **and** a decision: does confiscating unequip first, or take the slot with it?
- Genuine gap: you can't specify a weapon is ranged, so a shortbow in the current form doesn't take ammunition — **OPEN, and the biggest of these.** No `ranged`/`reach` field exists on the item type at all. The ENGINE side is already there — `roll:attack.ranged` and `roll:damage.ranged` are live selectors — so what is missing is the item model, the weapon form, the Equipment roller passing `sub: 'ranged'`, and ammo consumption on a hit. Also unblocks §19's `AmmoBonus` deletion, which is waiting on "what does active mean for a carried item" in `GUIDE_Codex_Deferred.md`. Touches dice-rolling, so it wants its own pass.
...
- **`ItemEffects.skills` (a flat per-skill bonus) has no authoring control.** Found while adding skill proficiency: it is readable by the engine and settable only by hand-editing JSON. Same inert-field class; a numeric column beside the new proficiency pickers would close it.
